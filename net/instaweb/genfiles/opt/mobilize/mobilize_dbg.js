...
pagespeed.MobNav.prototype.redrawHeader_ = function() {
  if (this.headerBar_) {
    var a = -1 == this.headerBarHeight_ ? Math.round(goog.style.getTransformedSize(this.headerBar_).height) : this.headerBarHeight_, b = goog.dom.getViewportSize().width, c = 1;
    b && (c = "hidden" == window.getComputedStyle(document.body).getPropertyValue("overflow-y") ? window.innerWidth / b : (window.innerWidth - goog.style.getScrollbarWidth()) / b);
    b = "scale(" + c.toString() + ")";
