...
pagespeed.MobUtil.ElementId = {CLICK_DETECTOR_DIV:"psmob-click-detector-div", CONFIG_IFRAME:"ps-hidden-iframe", NAV_PANEL:"psmob-nav-panel", HEADER_BAR:"psmob-header-bar", LOGO_SPAN:"psmob-logo-span", LOGO_IMAGE:"psmob-logo-image", MAP_BUTTON:"psmob-map-button", MAP_IMAGE:"psmob-map-image", MENU_BUTTON:"psmob-menu-button", PHONE_DIALER:"psmob-phone-dialer", PROGRESS_LOG:"ps-progress-log", PROGRESS_REMOVE:"ps-progress-remove", PROGRESS_SCRIM:"ps-progress-scrim", PROGRESS_SHOW_LOG:"ps-progress-show-log", 
PROGRESS_SPAN:"ps-progress-span", SPACER:"psmob-spacer"};
pagespeed.MobUtil.ElementClass = {HAMBURGER_DIV:"psmob-hamburger-div", HAMBURGER_LINE:"psmob-hamburger-line", LOGO_CHOOSER_CHOICE:"psmob-logo-chooser-choice", LOGO_CHOOSER_CONFIG_FRAGMENT:"psmob-logo-chooser-config-fragment", LOGO_CHOOSER_COLOR:"psmob-logo-chooser-color", LOGO_CHOOSER_SWAP:"psmob-logo-chooser-swap", LOGO_CHOOSER_COLUMN_HEADER:"psmob-logo-chooser-column-header", LOGO_CHOOSER_IMAGE:"psmob-logo-chooser-image", LOGO_CHOOSER_TABLE:"psmob-logo-chooser-table", MENU_EXPAND_ICON:"psmob-menu-expand-icon"};
...
pagespeed.MobColor.prototype.computeColors_ = function(a, b, c, d) {
  ...
  k = [0, 0, 0];
  for (f = l = g = 0;f < c;++f) {
    n = (d - 1) * c + f, h = 3 * n, k[0] += e[h], k[1] += e[h + 1], k[2] += e[h + 2], l += a[4 * n + 3], ++g;
  }
  if (l > 127.5 * g) {
    for (f = 0;3 > f;++f) {
  ...
  for (g = l;g <= d;++g) {
    for (f = a;f <= b;++f) {
      h = 3 * (g * c + f), m[n] = this.distance_(e.slice(h, h + 3), k), ++n;
    }
  }
  f = m.sort(function(a, b) {
    return a - b;
  });
  var p = Math.max(1, f[Math.floor(.75 * n)]), n = 0, m = [0, 0, 0];
  for (g = l;g <= d;++g) {
    for (f = a;f <= b;++f) {
      h = 3 * (g * c + f), this.distance_(e.slice(h, h + 3), k) >= p && (m[0] += e[h], m[1] += e[h + 1], m[2] += e[h + 2], ++n);
    }
  }
  if (0 < n) {
    for (f = 0;3 > f;++f) {
...
pagespeed.MobLayout.prototype.dontTouch = function(a) {
  if (!a) {
    return !0;
  }
  var b = a.nodeName.toUpperCase();
  return b == goog.dom.TagName.SCRIPT || b == goog.dom.TagName.STYLE || b == goog.dom.TagName.IFRAME || Boolean(a.id && this.dontTouchIds_[a.id]) || a.id == pagespeed.MobUtil.ElementId.NAV_PANEL || a.id == pagespeed.MobUtil.ElementId.HEADER_BAR || a.id == pagespeed.MobUtil.ElementId.SPACER || a.id == pagespeed.MobUtil.ElementId.LOGO_SPAN;
};
...
pagespeed.MobLayout.prototype.forEachMobilizableChild_ = function(a, b) {
  for (var c = a.firstElementChild;c;c = c.nextElementSibling) {
    this.dontTouch(c) || b.call(this, c);
  }
};
...
pagespeed.MobLayout.prototype.resizeVerticallyAndReturnBottom_ = function(a, b) {
  ...
  null != f && (d += f);
  for (var f = c + a.offsetHeight - 1, g = !1, h = !1, k, l = a.firstElementChild;l;l = l.nextElementSibling) {
    (k = window.getComputedStyle(l)) && ("absolute" != k.position || pagespeed.MobUtil.isOffScreen(k) || "0px" == k.getPropertyValue("height") || "hidden" == k.getPropertyValue("visibility") || (h = !0)), k = this.resizeVerticallyAndReturnBottom_(l, c), null != k && (g = !0, d = Math.max(d, k));
  }
...
pagespeed.MobLayout.prototype.isDataTable_ = function(a) {
  for (var b = 0, c = a.firstElementChild;c;c = c.nextElementSibling) {
    for (var d = c.firstElementChild;d;d = d.nextElementSibling) {
      ...
      for (e = d.firstElementChild;e;e = e.nextElementSibling) {
...
pagespeed.MobLayout.prototype.reorganizeTableQuirksMode_ = function(a, b) {
  ...
  var n = this.childElements_(a);
  for (c = 0;c < n.length;++c) {
    var m = this.childElements_(n[c]);
    for (d = 0;d < m.length;++d) {
      var p = this.childElements_(m[d]);
...
pagespeed.MobLayout.prototype.reorganizeTableNoQuirksMode_ = function(a, b) {
  ...
  for (c = a.firstElementChild;c;c = c.nextElementSibling) {
    for (pagespeed.MobUtil.removeProperty(c, "width"), pagespeed.MobUtil.setPropertyImportant(c, "max-width", "100%"), d = c.firstElementChild;d;d = d.nextElementSibling) {
      if (d.nodeName.toUpperCase() == goog.dom.TagName.TR) {
        for (pagespeed.MobUtil.removeProperty(d, "width"), pagespeed.MobUtil.setPropertyImportant(d, "max-width", "100%"), e = d.firstElementChild;e;e = e.nextElementSibling) {
          e.nodeName.toUpperCase() == goog.dom.TagName.TD && (pagespeed.MobUtil.setPropertyImportant(e, "max-width", "100%"), pagespeed.MobUtil.setPropertyImportant(e, "display", "inline-block"));
        }
      }
    }
  }
...
pagespeed.MobLayout.prototype.reallocateWidthToTableData_ = function(a) {
  ...
  if (a && (a = a.parentNode)) {
    ...
    for (b = a.firstElementChild;b;b = b.nextElementSibling) {
      b.nodeName.toUpperCase() == goog.dom.TagName.TD && ++c;
    }
    if (1 < c) {
      for (c = "width:" + Math.round(100 / c) + "%;", b = a.firstElementChild;b;b = b.nextElementSibling) {
        b.nodeName.toUpperCase() == goog.dom.TagName.TD && pagespeed.MobUtil.addStyles(b, c);
      }
    }
  }
};
...
pagespeed.MobLayout.prototype.stripFloats_ = function(a) {
  ...
  for (c = a.firstElementChild;c;c = c.nextElementSibling) {
    if (!this.dontTouch(c)) {
      ...
      e = this.stripFloats_(c);
      if ("fixed" != e && null != k && !this.dontTouch(c)) {
        "absolute" != e || pagespeed.MobUtil.isOffScreen(k) || pagespeed.MobUtil.setPropertyImportant(c, "position", "relative");
        e = k.getPropertyValue("float");
        var l = "right" == e;
        g = "inline-block";
        if (l || "left" == e) {
          l && "right" == k.getPropertyValue("clear") && (l = !1, g = "block", d && h && pagespeed.MobUtil.setPropertyImportant(d, "margin-bottom", "0px")), pagespeed.MobUtil.setPropertyImportant(c, "float", "none"), "none" != k.getPropertyValue("display") && pagespeed.MobUtil.setPropertyImportant(c, "display", g);
        }
        l && f.push(c);
        d = c;
        h = pagespeed.MobUtil.computedDimension(k, "margin-bottom");
        h = null != h && 0 > h;
      }
    }
  }
  ...
  if ("fixed" != window.getComputedStyle(a).getPropertyValue("position")) {
    ...
    for (c = a.firstElementChild;c;c = c.nextElementSibling) {
      if (!this.dontTouch(c)) {
        ...
        var f = b.getPropertyValue("position");
        "fixed" != f && "absolute" != f && 0 != c.offsetWidth && (d.push(c), e.push(b));
      }
    }
    for (var g = null, f = 0;f < d.length;++f) {
      c = d[f];
      b = f < d.length - 1 ? d[f + 1] : null;
      var h = c.offsetLeft + c.offsetWidth;
      (null == g || c.offsetLeft < g) && (null == b || b.offsetLeft < h) && (this.removeWidthConstraint_(c, e[f]), this.expandColumns_(c));
      a.getAttribute(pagespeed.MobLayout.NEGATIVE_BOTTOM_MARGIN_ATTR_) && (a.removeAttribute(pagespeed.MobLayout.NEGATIVE_BOTTOM_MARGIN_ATTR_), b = window.getComputedStyle(a), c = pagespeed.MobUtil.computedDimension(b, "height"), null != c && pagespeed.MobUtil.setPropertyImportant(a, "margin-bottom", "" + -c + "px"));
      g = h;
    }
  }
};
...
pagespeed.MobTheme.createMenuButton_ = function(a) {
  ...
  b.id = pagespeed.MobUtil.ElementId.MENU_BUTTON;
  var c = document.createElement(goog.dom.TagName.DIV);
  goog.dom.classlist.add(c, pagespeed.MobUtil.ElementClass.HAMBURGER_DIV);
  b.appendChild(c);
  a = pagespeed.MobUtil.colorNumbersToString(a);
  for (var d = 0;3 > d;++d) {
    var e = document.createElement(goog.dom.TagName.DIV);
    goog.dom.classlist.add(e, pagespeed.MobUtil.ElementClass.HAMBURGER_LINE);
...
pagespeed.MobTheme.installLogo = function(a) {
  ...
  a.anchorOrSpan.id = pagespeed.MobUtil.ElementId.LOGO_SPAN;
  a.logoElement.id = pagespeed.MobUtil.ElementId.LOGO_IMAGE;
...
pagespeed.MobTheme.extractTheme = function(a, b) {
  if (window.psConfigMode) {
    ...
    window.psMobBackgroundColor && window.psMobForegroundColor && !window.psMobPrecompute ? (c = null, psMobLogoUrl && (c = document.createElement(goog.dom.TagName.IMG), c.src = psMobLogoUrl, c = new pagespeed.MobLogo.LogoRecord(1, c)), c = pagespeed.MobTheme.createThemeData(c, window.psMobBackgroundColor, window.psMobForegroundColor), pagespeed.MobTheme.installLogo(c), b(c)) : (d = new pagespeed.MobTheme(b), a.run(goog.bind(d.logoComplete, d), 1));
  }
};
...
pagespeed.MobNav.prototype.findNavSections_ = function() {
  var a = [];
  if (this.navPanel_ = document.getElementById(pagespeed.MobUtil.ElementId.NAV_PANEL)) {
...
pagespeed.MobNav.prototype.findElementsToOffsetHelper_ = function(a, b) {
  if (a.className != pagespeed.MobUtil.ElementId.PROGRESS_SCRIM && !goog.string.startsWith(a.className, "psmob-") && !goog.string.startsWith(a.id, "psmob-")) {
...
pagespeed.MobNav.prototype.redrawHeader_ = function() {
  ...
  var a = Math.round(goog.style.getTransformedSize(this.headerBar_).height), b = goog.style.getSize(this.spacerDiv_).height;
  if (this.redrawNavCalled_ || a != b) {
    goog.dom.classlist.remove(this.headerBar_, "hide");
    var c = this.headerBar_.offsetHeight + "px";
    this.menuButton_ && (this.menuButton_.style.width = c);
    var d = this.dialer_.adjustHeight(c);
    this.mapButton_ && (this.mapButton_.style.width = c, d += this.headerBar_.offsetHeight);
    this.logoSpan_.style.left = c;
    this.logoSpan_.style.right = d + "px";
    this.spacerDiv_.style.height = a + "px";
    c = this.elementsToOffset_.getValues();
...
pagespeed.MobNav.prototype.addHeaderBar_ = function(a) {
  ...
  this.headerBar_ = document.getElementById(pagespeed.MobUtil.ElementId.HEADER_BAR);
...
pagespeed.MobNav.prototype.addMapNavigation_ = function(a) {
  var b = document.createElement(goog.dom.TagName.IMG);
  b.id = pagespeed.MobUtil.ElementId.MAP_IMAGE;
...
pagespeed.MobNav.prototype.addThemeColor_ = function(a) {
  ...
  b = "#" + pagespeed.MobUtil.ElementId.HEADER_BAR + " { background-color: " + b + "; }\n#" + pagespeed.MobUtil.ElementId.NAV_PANEL + " { background-color: " + a + "; }\n#" + pagespeed.MobUtil.ElementId.NAV_PANEL + " li { color: " + b + "; }\n#" + pagespeed.MobUtil.ElementId.NAV_PANEL + " a { color: " + b + "; }\n#" + pagespeed.MobUtil.ElementId.NAV_PANEL + " div { color: " + b + "; }\n";
...
pagespeed.MobNav.prototype.cleanupNavPanel_ = function() {
  for (var a = this.navPanel_.querySelectorAll("*"), b = [], c = document.getElementById(pagespeed.MobUtil.ElementId.LOGO_IMAGE), d = c ? c.src : "", c = 0, e;e = a[c];c++) {
  ...
  a = this.navPanel_.querySelectorAll("img:not(." + pagespeed.MobUtil.ElementClass.MENU_EXPAND_ICON + ")");
  for (c = 0;b = a[c];++c) {
  ...
  for (var b = 0, c;c = this.navSections_[b];b++) {
    if (c.parentNode) {
      ...
      for (var f = 0, g = d.length;f < g;f++) {
        ...
        if (h < k) {
          var l = document.createElement(goog.dom.TagName.LI);
          l.appendChild(document.createElement(goog.dom.TagName.DIV)).appendChild(document.createTextNode(d[f].textContent || d[f].innerText));
          e[e.length - 1].appendChild(l);
          h = document.createElement(goog.dom.TagName.UL);
          l.appendChild(h);
          e.push(h);
        } else {
          for (l = document.createElement(goog.dom.TagName.LI), e[e.length - 1].appendChild(l), l.appendChild(d[f].cloneNode(!0)), l = h - k;0 < l && 1 < e.length;) {
            e.pop(), l--;
          }
        }
      }
...
pagespeed.MobNav.prototype.addSubmenuArrows_ = function(a) {
  var b = this.navPanel_.getElementsByTagName(goog.dom.TagName.DIV), c = b.length;
...
pagespeed.MobNav.prototype.addNavPanel_ = function(a) {
  this.navPanel_ || this.constructNavPanel_();
...
pagespeed.MobNav.prototype.addNavButtonEvents_ = function() {
  this.navPanel_.firstChild.addEventListener(goog.events.EventType.CLICK, function(a) {
    a = goog.dom.isElement(a.target) && goog.dom.classlist.contains(a.target, pagespeed.MobUtil.ElementClass.MENU_EXPAND_ICON) ? a.target.parentNode : a.target;
...
pagespeed.MobNav.prototype.Run = function(a) {
  ...
  this.navDisabledForSite_() || !this.navPanel_ && 0 == this.navSections_.length || pagespeed.MobUtil.inFriendlyIframe() || (this.addNavPanel_(a), this.addMenuButtonEvents_(), this.addNavButtonEvents_());
  pagespeed.MobUtil.sendBeacon(pagespeed.MobUtil.BeaconEvents.NAV_DONE);
  window.addEventListener(goog.events.EventType.LOAD, goog.bind(this.redrawHeader_, this));
};
pagespeed.MobNav.prototype.updateHeaderBar = function(a, b) {
  if (b.logoImage()) {
    var c = pagespeed.MobUtil.colorNumbersToString(b.menuFrontColor), d = pagespeed.MobUtil.colorNumbersToString(b.menuBackColor), e = a.document.getElementById(pagespeed.MobUtil.ElementId.LOGO_IMAGE);
    ...
    if (d = a.document.getElementById(pagespeed.MobUtil.ElementId.MAP_IMAGE)) {
      d.src = pagespeed.MobUtil.synthesizeImage(pagespeed.MobNav.MAP_BUTTON, b.menuFrontColor);
    }
    for (var d = a.document.getElementsByClassName(pagespeed.MobUtil.ElementClass.HAMBURGER_LINE), e = 0, f;f = d[e];++e) {
...
pagespeed.MobNav.prototype.chooserShowCandidates = function(a) {
  ...
  if (this.logoChoicePopup_) {
    ...
    c.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_TABLE;
    var d = document.createElement(goog.dom.TagName.THEAD);
    c.appendChild(d);
    var e = document.createElement(goog.dom.TagName.TR);
    d.appendChild(e);
    e.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_COLUMN_HEADER;
    b().textContent = "Logo";
    ...
    for (var f = 0;f < a.length;++f) {
      ...
      e.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_CHOICE;
      d.appendChild(e);
      var g = a[f], h = pagespeed.MobTheme.synthesizeLogoSpan(g.logoRecord, g.background, g.foreground);
      b().appendChild(h.anchorOrSpan);
      h = h.logoElement;
      h.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_IMAGE;
      ...
      var k = b();
      k.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_COLOR;
      var l = document.createElement(goog.dom.TagName.IMG);
      l.className = pagespeed.MobUtil.ElementClass.LOGO_CHOOSER_SWAP;
