...
goog.debug.LogRecord.prototype.goog_debug_LogRecord_prototype$getMessage = function() {
  return this.msg_;
};
...
goog.debug.Logger.prototype.doLogRecord_ = function(a) {
  goog.debug.Logger.logToProfilers("log:" + a.goog_debug_LogRecord_prototype$getMessage());
  for (var b = this;b;) {
...
pagespeed.Graphs = function(a) {
  this.pagespeed_Graphs$xhr_ = a || new goog.net.XhrIo;
  this.psolMessages_ = [];
  this.secondRefreshStarted_ = this.firstRefreshStarted_ = this.autoRefresh_ = !1;
  this.chartCache_ = {};
  for (var b in pagespeed.Graphs.DisplayDiv) {
    document.getElementById(pagespeed.Graphs.DisplayDiv[b]).className = "pagespeed-hidden-offscreen";
  }
  a = document.createElement("table");
  a.id = "nav-bar";
  a.className = "pagespeed-sub-tabs";
  a.innerHTML = '<tr><td><a id="' + pagespeed.Graphs.DisplayMode.CACHE_APPLIED + '" href="javascript:void(0);">Per application cache stats</a> - </td><td><a id="' + pagespeed.Graphs.DisplayMode.CACHE_TYPE + '" href="javascript:void(0);">Per type cache stats</a> - </td><td><a id="' + pagespeed.Graphs.DisplayMode.IPRO + '" href="javascript:void(0);">IPRO status</a> - </td><td><a id="' + pagespeed.Graphs.DisplayMode.REWRITE_IMAGE + '" href="javascript:void(0);">Image rewriting</a> - </td><td><a id="' + 
  pagespeed.Graphs.DisplayMode.REALTIME + '" href="javascript:void(0);">Realtime</a></td></tr>';
  b = document.createElement("div");
  b.id = "ui-div";
  b.innerHTML = '<table id="ui-table" border=1 style="border-collapse: collapse;border-color:silver;"><tr valign="center"><td>Auto refresh (every 5 seconds): <input type="checkbox" id="auto-refresh" ' + (this.autoRefresh_ ? "checked" : "") + "></td></tr></table>";
  document.body.insertBefore(b, document.getElementById(pagespeed.Graphs.DisplayDiv.CACHE_APPLIED));
  document.body.insertBefore(a, document.getElementById("ui-div"));
};
pagespeed.Graphs.prototype.pagespeed_Graphs_prototype$show = function(a) {
  for (var b in pagespeed.Graphs.DisplayDiv) {
  ...
  for (b in pagespeed.Graphs.DisplayMode) {
    var d = document.getElementById(pagespeed.Graphs.DisplayMode[b]);
...
pagespeed.Graphs.prototype.parseLocation = function() {
  ...
  "" == a ? this.pagespeed_Graphs_prototype$show(pagespeed.Graphs.DisplayDiv.CACHE_APPLIED) : goog.object.contains(pagespeed.Graphs.DisplayDiv, a) && this.pagespeed_Graphs_prototype$show(a);
};
...
pagespeed.Graphs.prototype.createQueryUrl = function() {
  var a = new Date, b;
  b = "?json&start_time=" + (new Date(a - 864E5)).getTime();
  b += "&end_time=" + a.getTime();
  return b + "&granularity=5000";
};
pagespeed.Graphs.prototype.parseHistoricalMessages = function(a) {
  var b = JSON.parse(a);
  a = b.timestamps;
  for (var b = b.variables, c = 0;c < a.length;++c) {
    var d = [], e;
    for (e in b) {
      d.push({name:e, value:b[e][c]});
    }
    this.psolMessages_.push({messages:d, timeReceived:new Date(a[c])});
  }
};
pagespeed.Graphs.prototype.parseSnapshotMessages = function(a) {
  a = JSON.parse(a).variables;
  var b = [], c;
  for (c in a) {
    b.push({name:c, value:a[c]});
  }
  this.psolMessages_.push({messages:b, timeReceived:new Date});
};
pagespeed.Graphs.prototype.requestUrl = function() {
  var a = location.pathname, b = a.lastIndexOf("/", a.length - 2);
  return 0 < b ? a.substring(0, b) + "/stats_json" : a + "/stats_json";
};
pagespeed.Graphs.prototype.performRefresh = function() {
  if (!this.pagespeed_Graphs$xhr_.isActive()) {
    if (!this.firstRefreshStarted_) {
      this.firstRefreshStarted_ = !0, this.pagespeed_Graphs$xhr_.function__new_goog_net_XhrIo___goog_net_XmlHttpFactory_null_____undefined$send(this.createQueryUrl());
    } else {
      if (!this.secondRefreshStarted_ || this.autoRefresh_) {
        this.secondRefreshStarted_ = !0, this.pagespeed_Graphs$xhr_.function__new_goog_net_XhrIo___goog_net_XmlHttpFactory_null_____undefined$send(this.requestUrl());
      }
    }
  }
};
pagespeed.Graphs.prototype.parseAjaxResponse = function() {
  if (this.pagespeed_Graphs$xhr_.goog_net_XhrIo_prototype$isSuccess()) {
    var a = this.pagespeed_Graphs$xhr_.getResponseText();
    this.secondRefreshStarted_ ? (this.parseSnapshotMessages(a), 17280 < this.psolMessages_.length && this.psolMessages_.shift(), this.drawVisualization()) : (this.parseHistoricalMessages(a), window.setTimeout(goog.bind(this.performRefresh, this), 0));
  } else {
    console.log(this.pagespeed_Graphs$xhr_.getLastError());
  }
};
...
pagespeed.Graphs.screenData = function(a, b) {
  var c = !0;
  b.indexOf(a) ? c = !1 : 0 <= b.indexOf("cache_flush_timestamp_ms") ? c = !1 : 0 <= b.indexOf("cache_flush_count") ? c = !1 : 0 <= b.indexOf("cache_time_us") && (c = !1);
  return c;
};
pagespeed.Graphs.prototype.initChart = function(a, b, c, d) {
  if (this.chartCache_[b]) {
    c = this.chartCache_[b];
  } else {
    d = document.getElementById(d);
    "Loading Charts..." == d.textContent && (d.textContent = "");
    var e = document.createElement("div");
    "AnnotatedTimeLine" == c && (e.className = "pagespeed-graphs-chart");
    e.id = a;
    ...
    d.appendChild(a);
    d.appendChild(e);
    c = new google.visualization[c](e);
    this.chartCache_[b] = c;
  }
  return c;
};
pagespeed.Graphs.prototype.drawBarChart = function(a, b, c) {
  var d = "pagespeed-graphs-" + a;
  a += "_";
  b = this.initChart(d, b, "BarChart", c);
  for (var d = document.getElementById(d), e = [], f = new google.visualization.DataTable, g = goog.array.clone(this.psolMessages_[this.psolMessages_.length - 1].messages), h = c = 0;h < g.length;++h) {
    if (pagespeed.Graphs.screenData(a, g[h].name)) {
      ++c;
      var k = g[h].name.substring(a.length), k = k.replace(/_/g, " ");
  ...
  f.addRows(e);
  a = new google.visualization.DataView(f);
  a.setColumns([0, 1, {calc:function(a, b) {
    for (var c = 0, d = 0;d < a.getNumberOfRows();++d) {
      c += a.getValue(d, 1);
    }
    d = a.getValue(b, 1);
    return d.toString() + " (" + (100 * d / (0 == c ? 1 : c)).toFixed(2).toString() + "%)";
  }, type:"string", role:"annotation"}]);
  c = 40 * c + 10;
  d.style.height = c + 20;
  b.draw(a, {annotations:{alwaysOutside:!0, highContrast:!0, textStyle:{fontSize:12, color:"black"}}, hAxis:{direction:1}, vAxis:{textPosition:"out"}, legend:{position:"none"}, width:800, height:c, chartArea:{left:225, top:0, width:"60%", height:"80%"}});
};
pagespeed.Graphs.prototype.drawHistoryChart = function(a, b, c) {
  var d = "pagespeed-graphs-" + a;
  a += "_";
  b = this.initChart(d, b, "AnnotatedTimeLine", c);
  document.getElementById(d);
  d = [];
  c = new google.visualization.DataTable;
  c.addColumn("datetime", "Time");
  for (var e = !0, f = 0;f < this.psolMessages_.length;++f) {
    var g = goog.array.clone(this.psolMessages_[f].messages), h = [];
    h.push(this.psolMessages_[f].timeReceived);
    for (var k = 0;k < g.length;++k) {
      if (pagespeed.Graphs.screenData(a, g[k].name) && (h.push(Number(g[k].value)), e)) {
        var l = g[k].name.substring(a.length), l = l.replace(/_/g, " ");
  ...
  c.addRows(d);
  b.draw(c, pagespeed.Graphs.ANNOTATED_TIMELINE_OPTIONS_);
};
...
pagespeed.Graphs.TIMERANGE_ = 17280;
pagespeed.Graphs.Start = function() {
  goog.events.listen(window, "load", function() {
    var a = new pagespeed.Graphs;
    a.parseLocation();
    for (var b in pagespeed.Graphs.DisplayDiv) {
      goog.events.listen(document.getElementById(pagespeed.Graphs.DisplayMode[b]), "click", goog.bind(a.pagespeed_Graphs_prototype$show, a, pagespeed.Graphs.DisplayDiv[b]));
    }
    goog.events.listen(window, "hashchange", goog.bind(a.parseLocation, a));
    goog.events.listen(document.getElementById("auto-refresh"), "change", goog.bind(a.toggleAutorefresh, a));
    goog.events.listen(a.pagespeed_Graphs$xhr_, goog.net.EventType.COMPLETE, goog.bind(a.parseAjaxResponse, a));
    setInterval(goog.bind(a.performRefresh, a), 5E3);
    a.performRefresh();
  });
};
