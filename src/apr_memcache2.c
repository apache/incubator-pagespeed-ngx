...
#include "third_party/apr/src/include/apr_poll.h"
#include "third_party/apr/src/include/apr_version.h"
#include <assert.h>
#include <stdlib.h>

#define BUFFER_SIZE 512
struct apr_memcache2_conn_t
{
    char *buffer;
    apr_size_t blen;
    apr_pool_t *p;
...
static apr_status_t make_server_live(apr_memcache2_t *mc, apr_memcache2_server_t *ms)
{
...
APU_DECLARE(apr_status_t) apr_memcache2_add_server(apr_memcache2_t *mc, apr_memcache2_server_t *ms)
{
...
apr_memcache2_find_server_hash(apr_memcache2_t *mc, const apr_uint32_t hash)
{
    if (mc->server_func) {
        ...
        return apr_memcache2_find_server_hash_default(NULL, mc, hash);
    }
}
...
{
    apr_memcache2_server_t *ms = NULL;
...
APU_DECLARE(apr_memcache2_server_t *) apr_memcache2_find_server(apr_memcache2_t *mc, const char *host, apr_port_t port)
{
    int i;
...
static apr_status_t ms_bad_conn(apr_memcache2_server_t *ms, apr_memcache2_conn_t *conn)
{
#if APR_HAS_THREADS
...
static apr_status_t ms_release_conn(apr_memcache2_server_t *ms, apr_memcache2_conn_t *conn)
{
...
APU_DECLARE(apr_status_t) apr_memcache2_enable_server(apr_memcache2_t *mc, apr_memcache2_server_t *ms)
{
    apr_status_t rv = APR_SUCCESS;
...
{
    apr_memcache2_conn_t *conn = data;
    struct iovec vec[2];
    apr_size_t written;
...
#if APR_HAS_THREADS
    conn = malloc(sizeof( apr_memcache2_conn_t )); /* non-pool space! */
#else
    conn = apr_palloc(np, sizeof( apr_memcache2_conn_t ));
#endif
...
{
    apr_memcache2_conn_t *conn = (apr_memcache2_conn_t*)conn_;
                                                    ...
                                                    const apr_size_t data_len)
{
     ...
     * like this....
     */
    return ((apr_memcache2_hash_crc32(baton, data, data_len) >> 16) & 0x7fff);
}
                                            ...
                                            const apr_size_t data_len)
{
    if (mc->hash_func) {
        ...
        return apr_memcache2_hash_default(NULL, data, data_len);
    }
}

static apr_status_t get_server_line(apr_memcache2_conn_t *conn)
{
                                      ...
                                      apr_uint16_t flags)
{
    ...
    hash = apr_memcache2_hash(mc, key, key_size);

    ms = apr_memcache2_find_server_hash(mc, hash);
...
apr_memcache2_set(apr_memcache2_t *mc,
                 const char *key,
                 char *data,
                 const apr_size_t data_size,
                 apr_uint32_t timeout,
...
APU_DECLARE(apr_status_t)
apr_memcache2_add(apr_memcache2_t *mc,
                 const char *key,
                 char *data,
                 const apr_size_t data_size,
                 apr_uint32_t timeout,
...
APU_DECLARE(apr_status_t)
apr_memcache2_replace(apr_memcache2_t *mc,
                 const char *key,
                 char *data,
                 const apr_size_t data_size,
                 apr_uint32_t timeout,
                  ...
                  apr_uint16_t *flags_)
{
    ...
    hash = apr_memcache2_hash(mc, key, klen);
    ms = apr_memcache2_find_server_hash(mc, hash);
    if (ms == NULL)
        return APR_NOTFOUND;
    ...
    if (strncmp(MS_VALUE, conn->buffer, MS_VALUE_LEN) == 0) {
        char *flags;
        char *length;
        //oschaaf: declared but not used
        //char *start;
        char *last;
        apr_size_t len = 0;

        //oschaaf: set but no used
        //start = conn->buffer;
        ...
        //oschaaf: len is unsigned, so this will never be true:
        //if (len < 0)  {
        //    *new_length = 0;
        //    *baton = NULL;
        //}
...
APU_DECLARE(apr_status_t)
apr_memcache2_delete(apr_memcache2_t *mc,
                    const char *key,
                    apr_uint32_t timeout)
{
    apr_status_t rv;
...
static apr_status_t num_cmd_write(apr_memcache2_t *mc,
                                  const char *cmd,
...
APU_DECLARE(apr_status_t)
apr_memcache2_incr(apr_memcache2_t *mc,
                    const char *key,
                    apr_int32_t inc,
                    apr_uint32_t *new_value)
{
...
APU_DECLARE(apr_status_t)
apr_memcache2_decr(apr_memcache2_t *mc,
                    const char *key,
                    apr_int32_t inc,
                    apr_uint32_t *new_value)
{
...
APU_DECLARE(apr_status_t)
apr_memcache2_version(apr_memcache2_server_t *ms,
                  apr_pool_t *p,
                  char **baton)
{
    apr_status_t rv;
...
APU_DECLARE(void)
apr_memcache2_add_multget_key(apr_pool_t *data_pool,
                             const char* key,
                             apr_hash_t **values)
{
    ...
    value = apr_pcalloc(data_pool, sizeof(apr_memcache2_value_t));
...
APU_DECLARE(apr_status_t)
apr_memcache2_multgetp(apr_memcache2_t *mc,
                      ...
                      apr_hash_t *values)
{
    ...
    while (value_hash_index) {
        ...
        hash = apr_memcache2_hash(mc, value->key, klen);
        ms = apr_memcache2_find_server_hash(mc, hash);
        if (ms == NULL) {
            continue;
        }
    ...
    while (queries_sent) {
        ...
        for (i = 0; i < queries_recvd; i++) {
           ...
           if (strncmp(MS_VALUE, conn->buffer, MS_VALUE_LEN) == 0) {
               ...
               if (value) {
                   //oschaaf:len is unsigned, always true
                   //if (len >= 0)  {
                       apr_bucket_brigade *bbb;
                       apr_bucket *e;
...
static void update_stats(apr_pool_t *p, apr_memcache2_conn_t *conn,
...
APU_DECLARE(apr_status_t)
apr_memcache2_stats(apr_memcache2_server_t *ms,
                  apr_pool_t *p,
                  apr_memcache2_stats_t **stats)
{
    apr_memcache2_stats_t *ret;
    apr_status_t rv;
    apr_memcache2_conn_t *conn;
    apr_size_t written;
    struct iovec vec[2];
    ...
    ret = apr_pcalloc(p, sizeof(apr_memcache2_stats_t));
