...
var pagespeed = {Statistics:function(opt_xhr) {
  ...
  this.psolMessages_ = [];
  for (var messages = document.getElementById("stat").textContent.split("\n"), i = 0;i < messages.length;++i) {
...
pagespeed.Statistics.prototype.update = function() {
  ...
  for (var table = document.createElement("table"), i = 0;i < messages.length;++i) {
    var row = table.insertRow(-1), cell = row.insertCell(0);
...
pagespeed.Statistics.prototype.requestUrl = function() {
  var pathName = location.pathname, n = pathName.lastIndexOf("/", pathName.length - 2);
  return 0 < n ? pathName.substring(0, n) + "/stats_json" : pathName + "/stats_json";
};
