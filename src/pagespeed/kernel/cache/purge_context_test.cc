...
class PurgeContextTest : public testing::Test {
 ...
 protected:
  ...
  int64 LockContentionStart(PurgeContext::BoolCallback* callback) {
    ...
    // We don't check pending_purges_ in IsValid; the invalidation will
    // only be visible to purge_context1 when it can acquire the lock and
    // write its records.
    EXPECT_TRUE(purge_context1_->IsValid("b", now_ms - 1));
    EXPECT_TRUE(purge_context2_->IsValid("b", now_ms - 1));
...
TEST_F(PurgeContextTest, Empty) {
  EXPECT_TRUE(purge_context1_->IsValid("a", 500));
}

TEST_F(PurgeContextTest, InvalidationSharing) {
  purge_context1_->SetCachePurgeGlobalTimestampMs(400000, ExpectSuccess());
  purge_context1_->AddPurgeUrl("a", 500000, ExpectSuccess());
  EXPECT_FALSE(purge_context1_->IsValid("a", 500000));
  EXPECT_TRUE(purge_context1_->IsValid("a", 500001));
  EXPECT_FALSE(purge_context1_->IsValid("b", 400000));
  ...
  EXPECT_FALSE(purge_context2_->IsValid("a", 500000));
  EXPECT_TRUE(purge_context2_->IsValid("a", 500001));
  EXPECT_FALSE(purge_context2_->IsValid("b", 400000));
  ...
  EXPECT_FALSE(purge_context2_->IsValid("a", 500001));
  EXPECT_TRUE(purge_context1_->IsValid("a", 500001));  // have not polled yet
  scheduler_.AdvanceTimeMs(10 * Timer::kSecondMs);     // force poll
  EXPECT_FALSE(purge_context1_->IsValid("a", 500001));
  EXPECT_TRUE(purge_context1_->IsValid("b", 600001));
  EXPECT_FALSE(purge_context2_->IsValid("a", 500001));
  EXPECT_TRUE(purge_context2_->IsValid("b", 600001));

  // Now invalidate 'b' till 700k.
  purge_context2_->AddPurgeUrl("b", 700000, ExpectSuccess());
  EXPECT_FALSE(purge_context2_->IsValid("b", 700000));  // have not polled yet
  EXPECT_TRUE(purge_context1_->IsValid("b", 700000));   // have not polled yet
  scheduler_.AdvanceTimeMs(10 * Timer::kSecondMs);      // force poll
  EXPECT_FALSE(purge_context1_->IsValid("b", 700000));
  EXPECT_TRUE(purge_context1_->IsValid("b", 700001));
  EXPECT_FALSE(purge_context2_->IsValid("b", 700000));
  EXPECT_TRUE(purge_context2_->IsValid("b", 700001));
...
TEST_F(PurgeContextTest, EmptyPurgeFile) {
  ...
  EXPECT_FALSE(purge_context1_->IsValid("b", timer_.NowMs() - 1));
  EXPECT_TRUE(purge_context1_->IsValid("b", timer_.NowMs() + 1));
  EXPECT_EQ(0, file_parse_failures());
}
...
TEST_F(PurgeContextTest, LockContentionSuccess) {
  ...
  scheduler_.AdvanceTimeMs(10 * Timer::kSecondMs);
  EXPECT_FALSE(purge_context1_->IsValid("b", now_ms - 1));
  EXPECT_FALSE(purge_context2_->IsValid("b", now_ms - 1));
...
TEST_F(PurgeContextTest, InvalidTimestampInPurgeRecord) {
  ASSERT_TRUE(file_system_.WriteFile(
      ...
      &message_handler_));
  EXPECT_FALSE(purge_context1_->IsValid("a", 500));
  EXPECT_TRUE(purge_context1_->IsValid("a", 501));
