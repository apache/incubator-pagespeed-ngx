...
def ast_node_to_dict(node, dest=None, lookup={}, parent_key=""):
  if Node is None:
    return None
    ...
    for n in range(0, len(c), 2):
      ...
      dest[key] = ast_node_to_dict(c[n + 1], dest, lookup, parent_key + '.' + key)
      lookup[parent_key] = dest
  elif isinstance(node, List):
    dest=[]
    for index, child in enumerate(node.getChildren()):
      cn = ast_node_to_dict(child, None, lookup, parent_key + '.' + repr(index))
      dest.append( cn )
      lookup[parent_key] = dest
